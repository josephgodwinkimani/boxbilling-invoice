...
class ServiceTransaction implements InjectionAwareInterface
{
    ...
    public function getTransactions($transactionInvoice) 
    {
     
        $sql = "SELECT txn_id, note FROM transaction WHERE invoice_id = $transactionInvoice";
        $rows = $this->di['db']->getAll($sql);       
        $result = array();
        foreach ($rows as $row){
            $result[ $row['txn_id'] ] =  "Transaction ID.".$row['txn_id']." Paid by ". $row['note'];
        }
        //$json = json_encode($result,JSON_THROW_ON_ERROR);
        
        return implode(" ",$result);

        /*$details = $this->di['db']->getRow($sql, array(':cid'=>$transactionInvoice));
        $transaction = array();
        $transaction['note']          = $details['note'];
        $transaction['txn_id']       = $details['txn_id'];
        return json_encode($transaction,JSON_THROW_ON_ERROR);*/
